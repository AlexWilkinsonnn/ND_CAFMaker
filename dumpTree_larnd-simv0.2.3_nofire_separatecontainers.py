...
def dump(input_file, output_file):
    ...
    for jentry in range(entries):
        ...
        for containerName, hitSegments in event.SegmentDetectors:
            ...
            if containerName not in containerName_segments_list:
                containerName_segments_list[containerName] = []
            ...
            containerName_segments_list[containerName].append(segment)
    ...
    for containerName, curr_segments_list in containerName_segments_list.items():
        containerName_segments_list[containerName] = np.concatenate(curr_segments_list, axis=0)
    ...
    with h5py.File(output_file, "w") as f:
        ...
        for containerName, curr_segments_list in containerName_segments_list.items():
            f.create_dataset("segments_" + containerName, data=curr_segments_list)
