...
Taken from repo at commit 887da6f7ae457a058c6c882e8e5b5e1bdec6f15b (Aug 2022),
"""
...
def initHDF5File(output_file):
    with h5py.File(output_file, 'w') as f:
        f.create_dataset('trajectories', (0,), dtype=trajectories_dtype, maxshape=(None,), compression='gzip')
        f.create_dataset('segments', (0,), dtype=segments_dtype, maxshape=(None,), compression='gzip')
        f.create_dataset('vertices', (0,), dtype=vertices_dtype, maxshape=(None,), compression='gzip')
...
def dump(input_file, output_file):
    ...
    for jentry in tqdm(range(entries)):
        ...
        for containerName, hitSegments in event.SegmentDetectors:
            ...
            active = (containerName == "ArgonCube")
            ...
            for iHit, hitSegment in enumerate(hitSegments):
                ...
                segment[iHit]["vol_active"] = active
